...
def _OnFrame() -> None:
    ...
    _renderStatsWidget.Update(Renderer.renderStats.draws_count, Renderer.renderStats.vertex_count, Renderer.renderStats.draw_time,
                              _MAIN_PROCESS.memory_full_info().uss, Renderer.renderStats.video_memory_used, (
                                  Renderer.screenStats.width, Renderer.screenStats.height),
...
def _Setup() -> None:
    ...
    _infoFrame = tk.Frame(_window, bd=1, relief="solid",
                          bg=DEFAULT_IMGUI_BG_COLOR)
    _inspectorFrame = tk.Frame(
        _window, bd=1, relief="solid", bg=DEFAULT_IMGUI_BG_COLOR)
    _treeviewFrame = tk.Frame(
        _window, bd=1, relief="solid", bg=DEFAULT_IMGUI_BG_COLOR)

    _renderStatsWidget = RenderStatsWidget(_infoFrame)
    _contextInfoWidget = ContextInfoWidget(_infoFrame, Renderer.contextInfo.renderer, Renderer.contextInfo.version,
                                           Renderer.contextInfo.glsl_version, Renderer.contextInfo.vendor, Renderer.contextInfo.memory_available)
    ...
    _inspectorLabel = tk.Label(
        _inspectorFrame, text="Inspector", bd=0, bg=DEFAULT_IMGUI_BG_COLOR)


def _OpenScene() -> None:
    f = filedialog.askopenfilename(parent=_window, initialdir="./", title="Select file",
                                   filetypes=(("spyke scene files", "*.scn"), ("all files", "*.*")))
    if not f:
        return
...
def _SaveScene() -> None:
    f = filedialog.asksaveasfilename(parent=_window, initialdir="./", title="Select file",
                                     filetypes=(("spyke scene files", "*.scn"), ("all files", "*.*")))
    if not f:
        return
