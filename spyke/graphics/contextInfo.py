...
class ContextInfo:
    ...
    # TODO: Add enum for string names
    def _get_string(self, name: GL.GLenum) -> str:
        return GL.glGetString(name).decode('ascii')

    def get_info(self):
        self.renderer = self._get_string(GL.GL_RENDERER)
        self.version = self._get_string(GL.GL_VERSION)
        self.glsl_version = self._get_string(GL.GL_SHADING_LANGUAGE_VERSION)

        vendor = self._get_string(GL.GL_VENDOR).lower()
        if 'nvidia' in vendor:
        ...
        if not 'GL_EXT_direct_state_access' in self.extensions and \
            ...
            raise GraphicsException(
                'This graphics device doesn\'t meet the engine requirements.')
